...
pub(crate) struct JsonFile(pub HashMap<String, JsonFilter>);
...
impl JsonCompiler {
    ...
    pub fn compile<R: Read>(&self, reader: R) -> Result<HashMap<String, SeccompFilter>> {
        let filters: JsonFile = serde_json::from_reader(reader).map_err(Error::SerdeJson)?;
...
mod tests {
    ...
    use crate::backend::{
        ...
        SeccompCmpOp::*, SeccompCondition as Cond, SeccompFilter, SeccompRule, TargetArch,
    };
    ...
    fn test_error_messages() {
        assert_eq!(
            format!("{}", Error::Backend(BackendError::InvalidArgumentNumber)),
            "The seccomp rule contains an invalid argument index. Maximum index value: 5"
        );
        assert_eq!(
            format!(
                "{}",
                Error::SerdeJson(serde_json::from_str::<serde_json::Value>("").unwrap_err())
            ),
            format!(
                "Error parsing Json: {}",
